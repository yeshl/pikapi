...
_formatter = logging.Formatter(
    # fmt='[%(asctime)s] %(name)s <%(process)5d>%(threadName)s - [%(filename)s:%(lineno)s]%(levelname)s : %(message)s',
    fmt='[%(asctime)s] %(name)s <%(processName)s>%(threadName)s - [%(filename)s:%(lineno)s]%(levelname)s : %(message)s',
    datefmt="%Y-%m-%d - %H:%M:%S")
...
print('applog.py loaded logfile:%s' % logfile)
